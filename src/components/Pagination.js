  ...
  return (
    <nav className="bg-secondary text-light m-auto w-50 text-center">
      <ul className="pagination justify-content-center">
        ...
        {pageNumbers.map((number) => (<li key={number} className="page-item bg-secondary text-light"><a onClick={() => paginate(number)} className={`mouse page-link bg-${currentPage==number?"":"secondary"} text-light`}>{number}</a></li>))}
